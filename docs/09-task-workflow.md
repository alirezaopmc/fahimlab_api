...
2. **Branch**: Create a branch with conventional naming: `feat/area/description` or `fix/area/description` (e.g., `feat/blog/wagtail`, `fix/auth/session-handling`)
...
5. **Commit**: Commit each portion of related files atomically with conventional messages (e.g., `feat(people): add Person model and migrations`, `feat(people): add repositories and services`)
6. **Finish**: Update relevant docs in `docs/` when the task completes
